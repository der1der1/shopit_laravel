...
class HomeService
{
    ...
    public function getHomeDataWithCategorySearch($category)
    {
        $user = Auth::user();
        $marqee = $this->productRepository->getAllMarqee();
        $few_products = $this->productRepository->getRandomProducts(5);
        $products_category = $this->productRepository->getProductCategories();
        $allProducts = $this->productRepository->getProductsByCategory($category);
        $infos = $this->userRepository->parseUserInfo($user);
    ...
    public function searchProductsByWords(Request $request)
    {
        $searchWord = $request->search_word;
        $allProducts = $this->productRepository->searchProductsByCategory($searchWord);
