    ...
    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="text" id="pic_name" name="pic_name" value="{{ $product->pic_name }}">
            </div>
            
            <div class="form-group">
                <label for="selected">精選商品</label>
                <select id="selected" name="selected">
                    <option value="0" {{ $product->selected != '1' ? 'selected' : '' }}>否</option>
                    <option value="1" {{ $product->selected == '1' ? 'selected' : '' }}>是</option>
                </select>
            </div>
        </div>
        
        <div class="form-group">
            <label for="pic_dir">商品圖片（留空則不更新）</label>
            <input type="file" id="pic_dir" name="pic_dir" accept="image/*">
            @if($product->pic_dir)
            <div style="margin-top: 10px;">
                <p style="font-size: 12px; color: #7f8c8d;">目前圖片：</p>
                <img src="{{ asset($product->pic_dir) }}" alt="{{ $product->product_name }}" class="current-image">
            </div>
            @endif
        </div>
